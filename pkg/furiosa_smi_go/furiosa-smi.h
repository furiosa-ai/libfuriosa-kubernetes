...
typedef enum {
  ...
  FURIOSA_SMI_DEVICE_TO_DEVICE_LINK_TYPE_SOC = 70,
...
typedef struct {
  ...
  uint16_t major;
  uint16_t minor;
  FuriosaSmiFwVersion firmware_version;
  FuriosaSmiDriverVersion driver_version;
...
typedef struct {
  uint32_t count;
  FuriosaSmiDriverVersion driver_info[FURIOSA_SMI_MAX_DRIVER_INFO_SIZE];
